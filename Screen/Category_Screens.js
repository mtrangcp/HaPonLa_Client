...
const Category_Screens = () => {
  ...
  const renderBookCategories = () => {
    return bookCategories.map(category => (
      ...
      >
        <Text style={{ fontSize: 18, fontWeight: selectedTab === category ? 'bold' : 'normal' }}>{category}</Text>
      </TouchableOpacity>
    ));
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ paddingHorizontal: 20 }}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10 }}>{selectedTab}</Text>
        {/* Thêm danh sách sách theo thể loại ở đây */}
      </View>

      {/* Phần Tab Bar dưới cùng */}
      <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', padding: 20, borderTopWidth: 1, borderTopColor: '#ccc' }}>
        <TouchableOpacity>
